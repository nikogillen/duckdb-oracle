...
static unique_ptr<MergeIntoOperator> OraclePlanMergeIntoAction(
    OracleCatalog &catalog, ClientContext &context, LogicalMergeInto &op,
	...
	case MergeActionType::MERGE_DO_NOTHING:
		break;
	default:
		throw InternalException("Unsupported merge action type in Oracle MERGE");
	}
	return result;
}

PhysicalOperator &OracleCatalog::PlanMergeInto(ClientContext &context,
                                                 PhysicalPlanGenerator &planner,
                                                 LogicalMergeInto &op,
                                                 PhysicalOperator &plan) {
	if (op.return_chunk) {
		throw BinderException("RETURNING clause not yet supported for Oracle MERGE");
	}
	...
	for (auto &entry : op.actions) {
		...
		for (auto &action : entry.second) {
			planned_actions.push_back(
			    OraclePlanMergeIntoAction(*this, context, op, planner, *action, plan));
		}
		actions.emplace(entry.first, std::move(planned_actions));
	}

	auto &result = planner.Make<PhysicalMergeInto>(op.types, std::move(actions),
	                                                op.row_id_start, op.source_marker,
